...
$(document).ready(function(){
		...
		$('#cadastra').click(function(){
				...
				if ($('#nome').val() == '')          
    			{
	        		alert("Erro! Preencha corretamente o nome do protocolo.");
            		return false;                                 
    			}
			 
				if ($('#arquivo').val() == '')
    			{
			 		alert("Erro! O campo arquivo é obrigatório.");
             		return false;                                 
    			}
...
Session("usuario") = usuario
				...
				SQL = "select cod, nome, arquivo, obs from aulas where cod="&cod
				set rs  = server.CreateObject("ADODB.RecordSet")
				rs.Open SQL, objdb,3,1
				
				obs = replace(rs("obs"),"<br />",chr(13))
%>

<form method="post" action="editar_aula_ok.asp">
	<table style="font-family: Tahoma; font-size: 8pt;">
		<tr>
			<td colspan="4" class="area"><b>T&iacute;tulo:</b></td>
		</tr>
		<tr>
			<td colspan="4" class="area"><input type="text" name="nome" id="nome" size="51" value="<%=rs("nome")%>" /></td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="4" class="area">
				<textarea name="obs" id="obs" style="width: 350px; height: 60px;"><%=obs%></textarea>
			</td>
		</tr>	
		<tr>
			<td class="area">&nbsp;</td>
