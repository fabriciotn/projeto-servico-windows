...
%>
    ...
    if Upload.FileExists(caminho&"aulas/"& objNextFile.FileName) then
		Upload.DeleteFile(caminho&"aulas/"& objNextFile.FileName)
	end if

		
'---------------------------------------------------SALVA NA PASTA------------------------------------------------------------------------------------------------'

    objNextFile.Save caminho&"aulas\"	 'Salva o arquivo
...
SQL  = "UPDATE aulas SET arquivo = '"&objNextFile.FileName&"' WHERE cod = " & cod
ObjDb.execute SQL
