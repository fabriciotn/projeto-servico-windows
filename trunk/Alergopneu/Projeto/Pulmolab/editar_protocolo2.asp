...
<script type="text/javascript">
	...
	$(document).ready(function(){	
		...
		$('#link1').click(function(){
			...
			window.open("inserir_protocolo.asp?cod=<%=cod%>", "PULMOLAB" , "height=300,width=380,top=" + topo + ",left=" + 	esquerda + ",status=no,scrollbars=yes,menubar=no,toolbar=no,titlebar=no,location=no,directories=no,statusbar=no");
	
		});	
		
	$('#cadastra').click(function(){
	
		$.post('verifica_protocolo.asp',
		 { 
				 cod : <%=cod%>
...
<%
	...
	SQL = "SELECT arquivo, cod, obs FROM protocolos WHERE cod = " & cod
  ...
  <table style="font-family: Tahoma; font-size: 8pt;">
    ...
    <tr>
      <td class="area">
		...
		<%else%>
			<a id="abriArquivo" lang="<%=arquivo%>" style="cursor:pointer">
				<img alt="" width="60" height="50" src="img/protocolo.png" style="border: none;" title="Clique para abrir" />
			</a>
			<span style="padding-left: 10px;">Ações:&nbsp;
				<a href="delete_arquivo_protocolo.asp?cod=<%=cod%>" onclick="return confirm('Deseja realmente excluir este arquivo?')">
					<img style="border: none;" src="img/delete.png" title="Excluir arquivo" />
				</a>
			</span>
        <%end if%>
      </td>
